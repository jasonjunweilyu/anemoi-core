...
def _alltoallwrapper(output_list: list, input_list: list, group: ProcessGroup) -> None:
    """Wrapper around all_to_all for NCCL, MPI and Gloo backends.

    Gloo does not implement all_to_all directly, so we emulate it via point-to-point
    async exchanges between ranks.
    """
    ...
    if dist.get_backend(group) == "gloo":
        ...
        # Linear-shift partner schedule to exchange all shards once.
